...
- `src/models/` contains MCP access, OCR extraction, suggestion logic, local OCR fallback, and learning persistence.
...
Use environment variables for local paths and endpoints: `EVERNOTE_MCP_API_BASE`, `EVERNOTE_MCP_API_KEY`, `SCANSNAP_LEARNINGS_PATH`, `SCANSNAP_CLASSIFICATION_PATTERNS_PATH`, `SCANSNAP_CLASSIFICATION_RULES_PATH`, `SCANSNAP_SUGGESTIONS_CSV`, `SCANSNAP_LOCAL_OCR_DIR`, `SCANSNAP_OFFICE_CONVERTER_COMMAND`, and `SCANSNAP_OFFICE_PREVIEW_CACHE_DIR`. By default, UI corrections append to `private/SCANSNAP_CLASSIFICATION_PATTERNS.md`, optional private rules load from `private/classificationRules.js`, and converted Office preview PDFs are cached under `tmp/office-previews`.
